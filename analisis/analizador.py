...
def limpiar(df: pd.DataFrame) -> tuple[pd.DataFrame, InformeLimpieza]:
    ...
    # Garantizar tipos numéricos. En versiones recientes de pandas (>=2.2)
    # algunas columnas pueden quedar como StringArray al leer el CSV, lo que
    # rompe las comparaciones numéricas posteriores. Forzamos la conversión
    # antes de aplicar los filtros de rango.
    for col in df.columns:
        if col == "fecha_hora":
            continue
